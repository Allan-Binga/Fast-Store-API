...
export default function ProductCard({ product, onView }) {
  ...
  return <article className="group flex flex-col overflow-hidden rounded-sm border border-outline-variant bg-surface-container-lowest shadow-sm hover:shadow-md transition-shadow">
    ...
    <div className="flex flex-1 flex-col gap-3 p-4">
      ...
      <button disabled={busy || session.status === 'checking' || !inStock} onClick={() => inCart ? setPanel('cart') : addToCart(product)} className="rounded-sm bg-primary px-4 py-2.5 text-white hover:bg-secondary disabled:opacity-50">{!inStock ? 'Out of stock' : inCart ? 'View in cart' : 'Add to cart'}</button>
    </div>
  </article>
}
